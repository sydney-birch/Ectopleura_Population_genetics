...
def name_change(trim_dir):
    ...
    for item in os.scandir():
        ...
        if "_1P" in item.name: 
            ...
            print("line: mv {0} {1}".format(item.name, new_name))
            result = subprocess.run("mv {0} {1}".format(item.name, new_name), shell=True)
        
        elif "_2P" in item.name: 
            ...
            print("line: mv {0} {1}".format(item.name, new_name))
            result = subprocess.run("mv {0} {1}".format(item.name, new_name), shell=True)        
        elif "_1U" in item.name:
            continue
        elif "_2U" in item.name: 
            continue
